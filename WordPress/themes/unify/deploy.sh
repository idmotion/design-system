...
OLD_VERSION=$(grep "'version'" $API_DIR/unify-update-api.php | awk -F"'" '{print $4}')
...
\$data = array(
    'version' => '$VERSION',
    'download_url' => 'https://updates.idmotion.com.br/unify/unify-$VERSION.zip'
);
header('Content-Type: application/json');
echo json_encode(\$data);
?>" > "$API_DIR/unify-update-api.php"

# Atualiza a versão no arquivo style.css
sed -i "s/Version: $OLD_VERSION/Version: $VERSION/" "$THEME_DIR/style.css"

# Empacota o tema
ZIP_DESTINATION="../unify-$VERSION.zip"
cd $THEME_DIR
zip -r $ZIP_DESTINATION .
cd ..

# Copia o tema e o arquivo da API para o servidor
scp -P 65002 "unify-$VERSION.zip" $SERVER:/home/u871988871/public_html/updates/unify/
scp -P 65002 "unify-update-api.php" $SERVER:/home/u871988871/public_html/updates/unify/
